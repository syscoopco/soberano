-- FUNCTION: soberano.fn_Order_getTicket(integer, numeric, numeric, character, character varying)

-- DROP FUNCTION IF EXISTS soberano."fn_Order_getTicket"(integer, numeric, numeric, character, character varying);
...
BEGIN
	...
	IF counter IS NOT NULL THEN
		...
		FOR itemRecord IN SELECT iitem."This_has_Name" iname,
							...
							FROM soberano."Product" product
								...
								LEFT JOIN soberano."ProductCategory" parentpc
									ON parentproductcat."ProductCategoryHasProductCategoryId" = parentpc."ProductCategoryHasProductCategoryId"
							ORDER BY COALESCE(parentpc."This_is_shown_in_Position", pc."This_is_shown_in_Position") ASC, --this is to order by parent process run
									COALESCE(opr."This_is_an_addition_of_OrderProcessRun_with_ProcessRunHasProces", opr."ProcessRunHasProcessRunId") ASC,
									opr."ProcessRunHasProcessRunId" ASC
									/*pc."This_is_shown_in_Position" ASC, 
			...
			IF itemRecord.iqty > 0 THEN
				...
				IF compactPrinting THEN
					...
					itemQty := CAST(trunc(itemRecord.iqty) AS text) || /*' ' || itemRecord.unit ||*/ ' x';
		...
		ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																		   2,
																		   ARRAY['SUBTOTAL:', CAST(round(subtotal, 2) AS text)],
																		   ARRAY['l', 'r']) || chr(13);
		ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																		   2,
																		   ARRAY['-' || CAST(round(combinedDiscountRate, 2) AS text) || '%', CAST(round(subtotal - subtotal * combinedDiscountRate / 100, 2) AS text)],
																		   ARRAY['l', 'r']) || chr(13) || chr(13);
		...
		IF (providerName IS NULL) THEN
			...
			ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																			   2,
																			   ARRAY['RECARGO:' || ' ' || CAST(round(surchargevar, 2) AS text) || '%', '', CAST(round(surchargeAmount, 2) AS text)],
																			   ARRAY['l', 'r']) || chr(13) || chr(13);
		END IF;
		...
		ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																		   2,
																		   ARRAY['A PAGAR:', CAST(round(orderAmount, 2) AS text)],
																		   ARRAY['l', 'r']) || chr(13);	
		...
		ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																			   2,
																			   ARRAY['PAGADO:', CAST(round(paidAmount, 2) AS text)],
																			   ARRAY['l', 'r']) || chr(13);

		IF change > 0 THEN				
			ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																			   2,
																			   ARRAY['CAMBIO:', CAST(round(change, 2) AS text)],
																			   ARRAY['l', 'r']) || chr(13);
		END IF;
		
		ticket := ticket || soberano."fn_Utilities_formatColumnedTextLine"(printerProfile, 
																				   2,
																				   ARRAY['DEUDA:', CAST(round(orderAmount - collectedAmount, 2) AS text)],
																				   ARRAY['l', 'r']) || chr(13) || chr(13);

		ticket := COALESCE(pageHeader, '') || chr(13) || chr(13) || 
					'tt_ID_tt' || ' ' || CAST(orderid AS text) || chr(13) ||		
					/*'tt_TIME_tt' || ' ' ||*/ orderTime || chr(13) ||
					'tt_COUNTERS_tt' || ' ' || counter || chr(13) || ticket;
...
ALTER FUNCTION soberano."fn_Order_getTicket"(integer, numeric, numeric, character, character varying)
    OWNER TO postgres;
