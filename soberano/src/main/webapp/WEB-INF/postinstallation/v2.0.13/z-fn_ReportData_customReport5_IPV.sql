CREATE OR REPLACE FUNCTION soberano."z-fn_ReportData_customReport5_IPV"(
	lang character,
	fromd date,
	untild date,
	ccname character varying,
	loginname character varying)
    RETURNS TABLE("pName" character varying, "pStart" numeric, "pInput" numeric, "pMovement" numeric, "pLoss" numeric, "pOnSale" numeric, "pFinal" numeric, "pSold" numeric, "pPrice" numeric, "pAmount" numeric) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	IF (SELECT 'Manager' IN (SELECT metamodel."fn_User_getResponsibilities"(loginname))) THEN
			...
			RETURN QUERY /*(SELECT inamee, 
									openingg, 
									COALESCE(iQty, 0::numeric), 
									mQtyy, 
									lQtyy, 
									0::numeric, 
									endingg, 
									oQtyy, 
									pricee, 
									0::numeric FROM (SELECT iitemo."This_has_Name" inamee,
						...
						*/SELECT inamee, 
							...
							FROM (SELECT iitemo."This_has_Name" inamee,
												0.0::numeric,
												0.0::numeric,
												0.0::numeric,
												stockfrom.opening openingg, 
												...
												stockuntil.ending endingg,
												am."InventoryItemHasInventoryItemCode",
												pc."This_is_shown_in_Position" pcpos
											FROM soberano."Product" prod
												INNER JOIN soberano."ProductIsOfProductCategory" piopc
													ON piopc."ProductHasProductId" = prod."ProductHasProductId"
												INNER JOIN soberano."ProductCategory" pc
													ON pc."ProductCategoryHasProductCategoryId" = piopc."ProductCategoryHasProductCategoryId"
												INNER JOIN soberano."ProcessOutput" pro
													ON prod."InventoryItemHasInventoryItemCode" = pro."InventoryItemHasInventoryItemCode"
												INNER JOIN soberano."Process" process
													ON process."ProcessHasProcessId" = pro."ProcessHasProcessId"
												INNER JOIN soberano."ProcessInput" pri
													ON process."ProcessHasProcessId" = pri."ProcessHasProcessId"
												INNER JOIN soberano."AcquirableMaterial" am
													ON am."InventoryItemHasInventoryItemCode" = pri."InventoryItemHasInventoryItemCode"
												INNER JOIN soberano."InventoryItem" iitemo
											...
											GROUP BY iitemo."This_has_Name",
													...
													stockuntil.ending,
													am."InventoryItemHasInventoryItemCode",
													pc."This_is_shown_in_Position") sqq
											LEFT JOIN (SELECT SUM("This_is_called_with_Quantity") iQty,
																"This_is_called_with_InventoryItem_with_InventoryItemHasInventor" iItem
															FROM soberano."StockChange"
															WHERE "This_is_called_with_Quantity" > 0
