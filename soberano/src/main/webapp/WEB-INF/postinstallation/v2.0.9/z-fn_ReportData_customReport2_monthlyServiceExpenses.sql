CREATE OR REPLACE FUNCTION soberano."z-fn_ReportData_customReport2_monthlyServiceExpenses"(
	lang character,
	fromd date,
	untild date,
	loginname character varying)
    RETURNS TABLE(expensedate text, "iName" character varying, "iQty" numeric, "iUnit" character varying, amount numeric) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	IF ('Manager' IN (SELECT * from metamodel."fn_User_getResponsibilities"(loginname)))
		...
		OR EXISTS(SELECT * FROM metamodel."fn_EntityTypeInstance_getDecisions"(20, 1, loginname)) THEN
			...
			RETURN QUERY SELECT date_part('month', expense."This_is_at_DateTime")::text shift,
								"This_has_Name",
								1::numeric,
								'U'::character varying,
								SUM("Amount")
