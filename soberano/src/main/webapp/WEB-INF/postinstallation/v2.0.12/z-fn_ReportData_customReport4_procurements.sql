CREATE OR REPLACE FUNCTION soberano."z-fn_ReportData_customReport4_procurements"(
	lang character,
	fromd date,
	untild date,
	loginname character varying)
    RETURNS TABLE("expenseDate" text, "expenseProvider" character varying, "expenseReference" character varying, "expenseProduct" character varying, "productQuantity" double precision, "productUnit" character varying, "unitValue" double precision, currency character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	IF ('Manager' IN (SELECT * from metamodel."fn_User_getResponsibilities"(loginname)))
		...
		OR EXISTS(SELECT * FROM metamodel."fn_EntityTypeInstance_getDecisions"(20, 1, loginname)) THEN
			...
			--calc the shifts within which expenses were recorded
			...
			RETURN QUERY SELECT shift::text,
								prov."This_has_Name",
								"This_has_Reference",
								iitem."This_has_Name",
								"This_is_for_Quantity"::double precision,
								"Acronym",
								"Amount" / "This_is_for_Quantity"::double precision,
								"This_has_Currency_code"
							FROM soberano."ExpenseAmount" expamount
								INNER JOIN soberano."Currency" curr
									ON curr."CurrencyHasCurrencyId" = expamount."This_is_in_Currency_with_CurrencyHasCurrencyId"
								INNER JOIN soberano."MaterialExpense" matexp
								...
								INNER JOIN soberano."Expense" expense
									ON expense."ExpenseHasExpenseId" = matexp."ExpenseHasExpenseId"
										AND expense."This_is_at_DateTime" > pTime
										AND expense."This_is_at_DateTime" <= cTime
								INNER JOIN soberano."AcquisitionExpense" aexp
									ON aexp."ExpenseHasExpenseId" = expense."ExpenseHasExpenseId"
								INNER JOIN soberano."Provider" prov
									ON prov."ProviderHasProviderId" = aexp."This_pays_Provider_with_ProviderHasProviderId"
								INNER JOIN metamodel."EntityTypeInstance" eti
									ON eti."EntityTypeInstanceHasEntityTypeInstanceId" = matexp."This_is_identified_by_EntityTypeInstance_id"
							...
							ORDER BY shift ASC,
									...
									"This_has_Currency_code";									
	ELSE
		RETURN QUERY SELECT now()::date::text,
							''::character varying,
							''::character varying,
							''::character varying,
