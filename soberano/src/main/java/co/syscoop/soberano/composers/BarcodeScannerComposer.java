...
import co.syscoop.soberano.util.Barcode;
...
public class BarcodeScannerComposer extends SelectorComposer {
	...
	public void cmbItemToOrder_onChanging(Event event) {
		...
		if (Barcode.isAValidUPCorEANBarcode(barcode)) {
			
			Combobox cmbItemToOrder = (Combobox) event.getTarget();
			
			if ((Comboitem) cmbItemToOrder.getFirstChild() != null) {
    ...
    public void bcs_onDetect(Event event) {
		...
		try {
	        ...
	        Events.sendEvent(inputEventCombo);	        
