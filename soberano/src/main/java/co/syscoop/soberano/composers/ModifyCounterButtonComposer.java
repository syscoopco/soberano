...
public class ModifyCounterButtonComposer extends CounterFormComposer {
    ...
    public void btnApply_onClick() throws Throwable {
		try{
			...
			Counter counter = new Counter(((Intbox) incDetails.getParent().query("#intId")).getValue(),
										0,
			...
			if (counter.modify() == -1) {
				...
				treeObjects.getSelectedItem().setLabel(counter.getQualifiedName());
