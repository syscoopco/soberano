...
public class ServiceExpensesGridModel extends SoberanoAbstractListModel<Object>
{
	public ServiceExpensesGridModel() {
	...
	public int getSize() {
		...
		try {
			if (_size < 0)
				_size = new ServiceExpense().getCount();
			return _size;
		} 
	...
	public Object getElementAt(int index) {
		...
		if (_cache == null || index < _beginOffset || index >= _beginOffset + _cache.size()) {
			try {
				_beginOffset = index;
				_cache = new ServiceExpense().getAll(_orderBy == null?"recordingDate":_orderBy,
