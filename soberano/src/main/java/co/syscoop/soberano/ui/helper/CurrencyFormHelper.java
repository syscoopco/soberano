...
public class CurrencyFormHelper extends TrackedObjectFormHelper {
	...
	public void fillForm(Include incDetails, DefaultTreeNode<NodeData> data) throws SQLException {
		...
		ZKUtility.setValueWOValidation((Textbox) incDetails.query("#txtName"), currency.getName());
		ZKUtility.setValueWOValidation((Textbox) incDetails.query("#txtCode"), currency.getStringId());
		...
		if (currency.getPaymentProcessor() > 0) 
			ZKUtility.setValueWOValidation((Combobox) incDetails.query("#cmbPaymentProcessor"), currency.getPaymentProcessor());
		else
			((Combobox) incDetails.query("#cmbPaymentProcessor")).setSelectedIndex(-1);
		
		ZKUtility.setValueWOValidation((Decimalbox) incDetails.query("#decExchangeRate"), currency.getExchangeRate());
		ZKUtility.setValueWOValidation((Textbox) incDetails.query("#txtExchangeRateExpression"), currency.getExchangeRate().toString());
		ZKUtility.setValueWOValidation((Intbox) incDetails.query("#intPosition"), currency.getPosition());
	}
	...
	public void cleanForm(Include incDetails) {
		...
		ZKUtility.setValueWOValidation((Decimalbox) incDetails.query("#decExchangeRate"), new BigDecimal(1.0));
	}
