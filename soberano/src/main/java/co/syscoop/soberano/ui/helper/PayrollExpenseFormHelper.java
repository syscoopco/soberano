...
import co.syscoop.soberano.models.PayrollExpensesGridModel;

public class PayrollExpenseFormHelper extends BusinessActivityTrackedObjectFormHelper {
	...
	public void cleanForm(Box boxDetails) {
		...
		((Grid) boxDetails.getParent().getParent().getParent().query("center").query("window").query("grid")).setModel(new PayrollExpensesGridModel());
	}
	...
	public Integer recordFromForm(Box boxDetails) throws Exception {
		...
		if (cmbiWorker == null ||
			...
			Integer qryResult = (new PayrollExpense(dateExpenseDate.getValue(),
