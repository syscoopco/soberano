...
class O4_ProductCategoryTest_search_showingAll {
	
	final private Integer categoryCount = 10; //number of categories in this testing point
	final private Integer baseId = 1000; //the id of the first added category is baseId + 1. the last one's id is basedId + categoryCount
	...
	private void testForAllowedUser(Integer userSuffix) {
		...
		String qualifiedNamePattern = "cat#suffix#";
		...
		TestUtilityCode.testSearchCombobox("/product_categories.zul", categoryCount, userSuffix, baseId, qualifiedNamePattern);
		
		//showing all in tree
		TestUtilityCode.testShowingAllTree("/product_categories.zul", categoryCount, userSuffix, baseId, qualifiedNamePattern);
	};
	...
	private void testForDisallowedUser(Integer userSuffix) {
		...
		assertEquals(0,
					...
					userName + " must not have access to any categories. However, it sees " 
							+ cmbIntelliSearch.as(Combobox.class).getModel().getSize() 
							+ " categories in search combobox, and " 
