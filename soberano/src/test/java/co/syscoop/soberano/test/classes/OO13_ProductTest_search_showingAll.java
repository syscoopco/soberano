...
class OO13_ProductTest_search_showingAll {
	...
	final private Integer productCount = 9; //number of objects in this testing point
	final private Integer baseId = 1000; //the id of the first added object is baseId + 1. the last one's id is basedId + productCount
	...
	private void testForAllowedUser(Integer userSuffix) {
		...
		String qualifiedNamePattern = "product#suffix# : p#suffix#";
		...
		TestUtilityCode.testSearchCombobox("/products.zul", productCount, userSuffix, baseId, qualifiedNamePattern);
		
		//showing all in tree
		TestUtilityCode.testShowingAllTree("/products.zul", productCount, userSuffix, baseId, qualifiedNamePattern);
	};
	...
	private void testForDisallowedUser(Integer userSuffix) {
		...
		assertEquals(0,
					...
					userName + " must not have access to any products. However, it sees " 
							+ cmbIntelliSearch.as(Combobox.class).getModel().getSize() 
							+ " products in search combobox, and " 
