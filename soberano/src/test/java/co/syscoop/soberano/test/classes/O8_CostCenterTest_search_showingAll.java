...
class O8_CostCenterTest_search_showingAll {
	...
	final private Integer costCenterCount = 10; //number of cost centers in this testing point
	final private Integer baseId = 1000; //the id of the first added cost center is baseId + 1. the last one's id is basedId + costCenterCount
	...
	private void testForAllowedUser(Integer userSuffix) {
		...
		String qualifiedNamePattern = "cc#suffix#";
		...
		TestUtilityCode.testSearchCombobox("/cost_centers.zul", costCenterCount, userSuffix, baseId, qualifiedNamePattern);
		
		//showing all in tree
		TestUtilityCode.testShowingAllTree("/cost_centers.zul", costCenterCount, userSuffix, baseId, qualifiedNamePattern);
	};
	...
	private void testForDisallowedUser(Integer userSuffix) {
		...
		assertEquals(0,
					...
					userName + " must not have access to any cost centers. However, it sees " 
							+ cmbIntelliSearch.as(Combobox.class).getModel().getSize() 
							+ " cost centers in search combobox, and " 
