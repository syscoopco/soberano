...
class O4_ProviderTest_search_showingAll {
	...
	final private Integer providerCount = 8; //number of providers in this testing point
	final private Integer baseId = 1000; //the id of the first added provider is baseId + 1. the last one's id is basedId + providerCount
	...
	private void testForAllowedUser(Integer userSuffix) {
		...
		String qualifiedNamePattern = "prov#suffix#";
		...
		TestUtilityCode.testSearchCombobox("/providers.zul", providerCount, userSuffix, baseId, qualifiedNamePattern);
		
		//showing all in tree
		TestUtilityCode.testShowingAllTree("/providers.zul", providerCount, userSuffix, baseId, qualifiedNamePattern);
	};
	...
	private void testForDisallowedUser(Integer userSuffix) {
		...
		assertEquals(0,
					...
					userName + " must not have access to any providers. However, it sees " 
							+ cmbIntelliSearch.as(Combobox.class).getModel().getSize() 
							+ " providers in search combobox, and " 
